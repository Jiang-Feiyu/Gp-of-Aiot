...
static complex_t csi_complex_buffer[CSI_BUFFER_LENGTH/2]; // 用于存储转换为复数的CSI数据
 // [1] END OF YOUR CODE
 ...
 // 添加特征提取函数
static float extract_feature(const complex_t* buffer, int length, int feature_idx) {
    // 获取CSI幅值
    ...
    for (int i = 0; i < length; i++) {
        float real = buffer[i].real;
        float imag = buffer[i].imag;
        ...
        case 0: // 方差
            {
                ...
                float variance = 0.0f;
                for (int i = 0; i < length; i++) {
                    float diff = amplitudes[i] - mean;
                    variance += diff * diff;
                }
                variance /= length;
                feature_value = variance;
            }
            break;
        ...
        case 1: // 平均幅度变化
            {
                ...
                feature_value = sum_diff / (length - 1);
            }
            break;
        ...
        case 2: // 幅度标准差
            {
                ...
                variance /= length;
                feature_value = sqrtf(variance); // 标准差
            }
            break;
        ...
        case 3: // 峰度
            {
                ...
                for (int i = 0; i < length; i++) {
                    float diff = amplitudes[i] - mean;
                    kurtosis += powf(diff, 4);
                }
                kurtosis = kurtosis / (length * powf(variance, 2)) - 3.0f;
                feature_value = kurtosis;
            }
            break;
        ...
        case 4: // 偏度
            {
                ...
                for (int i = 0; i < length; i++) {
                    float diff = amplitudes[i] - mean;
                    skewness += powf(diff, 3);
                }
                skewness = skewness / (length * powf(std_dev, 3));
                feature_value = skewness;
            }
            break;
        ...
        case 5: // 前后半段差异
            {
                ...
                for (int i = 0; i < half; i++) {
                    first_half_mean += amplitudes[i];
                }
                first_half_mean /= half;
                
                for (int i = half; i < length; i++) {
                    second_half_mean += amplitudes[i];
                }
                second_half_mean /= (length - half);
                
                feature_value = fabsf(first_half_mean - second_half_mean);
            }
            break;
        ...
        case 6: // 最大幅度变化
            {
                float max_diff = 0.0f;
                for (int i = 1; i < length; i++) {
                    float diff = fabsf(amplitudes[i] - amplitudes[i-1]);
                    if (diff > max_diff) {
                        max_diff = diff;
                    }
                }
                feature_value = max_diff;
            }
            break;
        ...
        case 7: // 信号能量
            {
                ...
                feature_value = energy / length;
            }
            break;
            
        default:
            ESP_LOGE(TAG, "未知特征索引: %d", feature_idx);
            feature_value = 0.0f;
    }
 ...
 bool motion_detection() {
    ESP_LOGI(TAG, "执行运动检测算法");
    
    // 检查缓冲区中是否有足够的数据
    if (CSI_Q_INDEX < 100) {
        ESP_LOGI(TAG, "CSI数据不足，无法执行运动检测");
        return false;
    }
    
    // 将CSI数据转换为复数形式 (I和Q分量)
    int complex_count = 0;
    for (int i = 0; i < CSI_Q_INDEX - 1; i += 2) {
        if (complex_count >= CSI_BUFFER_LENGTH/2) break;
        csi_complex_buffer[complex_count].real = (float)CSI_Q[i];
        csi_complex_buffer[complex_count].imag = (float)CSI_Q[i+1];
        complex_count++;
    }
    
    // 提取特征
    float feature_value = extract_feature(csi_complex_buffer, complex_count, MODEL_FEATURE_IDX);
    
    // 使用模型阈值进行预测
    bool detection_result = (feature_value > MODEL_THRESHOLD) ? true : false;
    
    // 如果需要反转预测结果
    if (MODEL_INVERT_PREDICTION) {
        detection_result = !detection_result;
    }
    
    ESP_LOGI(TAG, "运动检测结果: %s (特征值: %.4f, 阈值: %.4f)", 
             detection_result ? "有运动" : "无运动", 
             feature_value, MODEL_THRESHOLD);
 ...
 {   
     ...
     // 2. 执行运动检测算法
     if (CSI_Q_INDEX >= 200) {  // 确保缓冲区有足够的数据
         bool motion_result = motion_detection();
         
         // 构造MQTT消息，包含CSI数据和运动检测结果
         char csi_message[1024]; // 注意缓冲区大小需要足够大以存储CSI数据
         int offset = 0;
         
         // 添加运动检测结果
         offset += snprintf(csi_message + offset, sizeof(csi_message) - offset, 
                          "MOTION_DETECTION: %d, CSI_DATA: ", motion_result ? 1 : 0);
         ...
         // 打印日志以调试
         ESP_LOGI(TAG, "发送的数据: %s", csi_message);
         
         // 调用mqtt_send函数发送数据
         mqtt_send(csi_message);
     } else {
         // 如果数据不足，仍然发送原始CSI数据
         char csi_message[1024]; 
         int offset = 0;
         ...
         // 打印日志以调试
         ESP_LOGI(TAG, "数据不足，仅发送CSI数据: %s", csi_message);
         
         // 调用mqtt_send函数发送数据
         mqtt_send(csi_message);
     }
     
     // 如果CSI缓冲区接近填满，清空一部分以防止溢出
     if (CSI_Q_INDEX >= CSI_BUFFER_LENGTH - 50) {
         // 保留最后100个样本
         memmove(CSI_Q, CSI_Q + (CSI_Q_INDEX - 100), 100 * sizeof(int16_t));
         CSI_Q_INDEX = 100;
         ESP_LOGI(TAG, "CSI缓冲区已重置，保留最后100个样本");
     }
